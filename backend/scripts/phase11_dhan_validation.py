...
async def run_experiment():
    print("==================================================")
    print("PHASE 11 — DHANHQ REAL MARKET DATA & PAPER-TRADING VALIDATION")
    print("==================================================")
    
    if not settings.DHAN_CLIENT_ID or not settings.DHAN_ACCESS_TOKEN:
        print("\nBLOCKED — Dhan credentials unavailable.")
        print("Please configure DHAN_CLIENT_ID and DHAN_ACCESS_TOKEN in backend/.env to run this validation.")
        return
    ...
    print(f"\n1. Ingesting Real Data for {symbol} ({timeframe}) from DhanHQ")
    try:
    ...
    print(f"Retrieved {len(df)} rows. Range: {df['timestamp'].min()} to {df['timestamp'].max()}")
